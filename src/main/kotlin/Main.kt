...
import ru.netology.coroutines.dto.PostsWithComments
        ...
        launch {
            try {
                val posts = getPosts(client)
                    .map { post ->
                        async {
                            PostsWithComments(post, getComments(client, post.id), getAuthor(client, post.authorId))
                        }
                    }.awaitAll()
                println(posts)
            } catch (e: Exception) {
...
suspend fun getCommentsWithAuthors(client: OkHttpClient): List<Post> =
    makeRequest("$BASE_URL/api/slow/posts", client, object : TypeToken<List<Post>>() {})